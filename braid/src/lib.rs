...
pub fn braid_start(name: &str) -> Result<()> {
    ...
    if std::env::var_os("RUST_LOG").is_none() {
        std::env::set_var("RUST_LOG", "braid=info,flydra2=info,flydra2_mainbrain=info,strand_cam=info,flydra_feature_detector=info,rt_image_viewer=info,flydra1_triggerbox=info,error");
    }
