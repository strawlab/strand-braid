...
traj3d_filename = os.path.join(dirname,'kalman_estimates.csv')
if os.path.exists(traj3d_filename):
    kest_df = pd.read_csv(traj3d_filename)
else:
    print('WARNING: no 3D data, unable to plot 3D')
    kest_df = None

fig = plt.figure()
ax = fig.add_subplot(211)
if kest_df is not None:
