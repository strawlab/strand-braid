...
pub fn mask_from_points(viewport_points: &[(f64,f64)]) -> Mask {
    use ncollide2d::shape::{ConvexPolygon, Compound, ShapeHandle};

    let points: Vec<_> = viewport_points.iter()
        .map(|p| delaunator::Point { x: p.0, y: p.1}).collect();

    let delaun = delaunator::triangulate(&points).expect("No triangulation exists.");
    let delta = nalgebra::Isometry2::identity();

    let shapes: Vec<_> = (&(delaun.triangles)).chunks(3).map(|idxs| {
        let a = &points[idxs[0]];
        let b = &points[idxs[1]];
        let c = &points[idxs[2]];
        let a = to_na(a);
