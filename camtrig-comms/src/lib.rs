...
pub enum FromDevice {
    ...
    CounterInfoResponse((u8, CounterInfo)),
    TimerResponse((u64, u16)),
}
...
impl TriggerState {
    pub const fn default() -> TriggerState {
        TriggerState {
            running: Running::ConstantFreq(100),
        }
    }
}
...
const DEFAULT_PULSE_TRAIN_PARAMS: PulseTrainParams = PulseTrainParams {
    n_pulses: 1,
    pulse_dur_ticks: 100,
    // pulse_freq_hz: 10,
};
...
impl Default for PulseTrainParams {
    fn default() -> Self {
        DEFAULT_PULSE_TRAIN_PARAMS.clone()
    }
}
...
impl std::fmt::Display for OnState {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::result::Result<(), std::fmt::Error> {
        match self {
            &OnState::PulseTrain(_) => write!(fmt, "Single Pulse"), // TODO extend to > 1 pulses
...
const ON_STATE_VARIANTS: [OnState; 3] = [
    OnState::Off,
    OnState::ConstantOn,
    OnState::PulseTrain(DEFAULT_PULSE_TRAIN_PARAMS),
];
...
#[cfg(test)]
impl quickcheck::Arbitrary for TriggerState {
    fn arbitrary<G: quickcheck::Gen>(g: &mut G) -> TriggerState {
        TriggerState {
            running: Running::arbitrary(g),
        }
    }
}
...
#[cfg(test)]
impl quickcheck::Arbitrary for Running {
    fn arbitrary<G: quickcheck::Gen>(g: &mut G) -> Running {
        match g.gen_range(0, 2) {
            0 => Running::Stopped,
            1 => Running::ConstantFreq(g.gen()),
            _ => unreachable!(),
        }
    }
}
...
mod tests {
    ...
    use {ChannelState, DeviceState, OnState, Running, TriggerState};
    ...
    quickcheck! {
        fn rt_trigger_state(val: TriggerState) -> bool {
            rt_val(&val)
        }
    }
    ...
    quickcheck! {
        fn rt_running(val: Running) -> bool {
            rt_val(&val)
        }
    }
}
