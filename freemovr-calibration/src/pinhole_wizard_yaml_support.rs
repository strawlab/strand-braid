...
use crate::{Result, DisplayGeometry};
use ncollide_geom::{Mask, mask_from_points};
use crate::types::{Display, SimpleDisplay, VirtualDisplay, VirtualDisplayName,
    SimpleUVCorrespondance, UVCorrespondance, VDispInfo};
...
impl SimplePinholeInputFile {
    pub fn to_orig(self) -> PinholeInputFile {
        PinholeInputFile {
            checkerboards: None,
            uv_display_points: self.uv_display_points.into_iter().map(|x| x.to_orig(DEFAULT_VDISP)).collect(),
...
impl PinholeCalib for SimplePinholeNoFile {
    ...
    fn uv_display_points(&self) -> Vec<UVCorrespondance> {
        self.uv_display_points.clone().into_iter().map(|x| x.to_orig(DEFAULT_VDISP)).collect()
    }
...
impl Geom {
    pub(crate) fn load_geom<P: AsRef<Path>>(&self, yaml_dir: P) -> Result<Box<dyn DisplayGeometry>> {
        match self {
    ...
    pub(crate) fn as_trimesh<P: AsRef<Path>>(&self, yaml_dir: P) -> Result<TriMeshGeom> {
        match self {
            Geom::Sphere(_) => {
                Err(crate::error::Error::new(crate::error::ErrorKind::RequiredTriMesh))
            },
            Geom::FromFile(ff) => {
                ...
                let file = std::fs::File::open(&obj_fname)
                    .context(format!("loading geometry from file: {:?} (yaml dir is \"{}\"",
                        ff.filename, yaml_dir.as_ref().display()))?;
...
pub fn parse_obj_from_reader<R: std::io::Read>(mut file: R, fname: Option<&str>) -> Result<TriMeshGeom> {
    let mut buf = Vec::new();
    ...
    if objects.len() != 1 {
        ...
        return Err(crate::error::Error::new(ObjMustHaveExactlyOneObject(objects.len())));
    }
...
impl DisplayGeometry for LoadedSphere {
    fn texcoord2worldcoord(&self, tc: &Point2<f64>) -> Option<Point3<f64>> {
        ...
        Some(Point3::new(x + self.center[0],y + self.center[1], z + self.center[2]))
    }
...
fn get_uvs_trimesh(mesh: &ncollide3d::shape::TriMesh<f64>) -> Option<ncollide3d::shape::TriMesh<f64>> {
    if let Some(ref uvs) = mesh.uvs() {
        let indices = mesh.indices();
        let coords = uvs.iter().map(|uv| Point3::new( uv[0], uv[1], 0.0 )).collect();
        Some(ncollide3d::shape::TriMesh::<f64>::new( coords, indices, None))
    } else {
        None
    }
}
...
impl std::fmt::Debug for TriMeshGeom {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "TriMeshGeom {{ original_fname: {:?} }}",
            self.original_fname)
    }
}

impl TriMeshGeom {
    pub fn new(mesh: &ncollide3d::shape::TriMesh<f64>, original_fname: Option<String>) -> Result<Self> {
...
impl DisplayGeometry for TriMeshGeom {
    fn texcoord2worldcoord(&self, tc: &Point2<f64>) -> Option<Point3<f64>> {
        ...
        let (pp, (tri_idx, loc)) = self.uvs.project_point_with_location(&Isometry3::identity(), &point, true);
        ...
        let wc = match loc {
            ...
            TrianglePointLocation::OnEdge(edge_num,bcoords) => {
                ...
                let worldcoord = vertices[idx0].coords*bcoords[0] + vertices[idx1].coords*bcoords[1];
                worldcoord.into()
            },
...
pub fn solve_no_distortion_display_camera<D: PinholeCalib>(data: &D, geom: &dyn DisplayGeometry, epsilon: f64) -> Result<Vec<(VirtualDisplayName,mvg::Camera<f64>)>> {
    ...
    for vdisp in data.virtual_displays().iter() {
        ...
        let this_vdisp_points = uv_display_points.iter()
        ...
        info!("computing for virtual display: {} with {} points",
            vdisp.id.0, this_vdisp_corr.len());
...
fn mean_forward(cam: &mvg::Camera<f64>, pts: &[dlt::CorrespondingPoint<f64>]) -> f64 {
    ...
    for pt in pts {
        ...
        let world_pt = PointWorldFrame{ coords: Point3::from_slice(&o)};
...
pub fn compute_mask<D: PinholeCalib>(data: &D, id: &VirtualDisplayName) -> Result<Mask> {
    for vdisp in data.virtual_displays().iter() {
        ...
        if &vdisp.id == id {
            let points: Vec<_> = vdisp.viewport.iter()
...
pub fn merge_vdisps<D: PinholeCalib>(data: &D, vdisp_data: &[&VDispInfo], show_mask: bool) -> Vec<f64> {
...
mod tests {
    ...
    fn sphere_texcoord_worldcoord_roundtrip() {
        let sg = SphereGeom {
            center: WorldCoord {x: 0.1, y: 2.3, z: 4.5},
            radius: 1.234,
        };
    ...
    fn trimesh_dense_interp() {
        ...
        let indices: Vec<Point3<usize>> = vec![
            Point3::new(0, 1, 2),
        ];
...
// TODO: test yaml file which specifies display geometry in .obj file
    ...
    fn obj_test() {
        ...
        let cam = mvg::Camera::new(2*cx as usize, 2*cy as usize, extrinsics, intrinsics.into()).unwrap();
        ...
        let (texcoords, nchan) = compute_image_for_camera_view(
            &cam, Computable::TexCoords, &geom, &mask ).unwrap();

        crate::debug_image(&texcoords, cam.height() as u32, cam.width() as u32, nchan,
            "square.jpg", None).unwrap();
    }
