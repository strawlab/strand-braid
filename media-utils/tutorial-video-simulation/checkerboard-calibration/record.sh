...
# requirement). Unlike strand-cam-intro/braid-intro, this isn't regenerating
# a pre-existing tutorial video (no "Video_3.mp4" exists in this repo) --
# it's new content.
#
...
# way the other scenarios' own fallback offsets were tuned.
...
# comment above). Bounded to CHECKERBOARD_DONE_WAIT_TRIES tries * 1s (computed
...
if wait_for_file_newer_than "$CHECKERBOARD_CAL_YAML" "$CHECKERBOARD_CAL_YAML_BASELINE_MTIME" 15 2; then
    ...
    # singleton isolation problems). See open_file_navigator's own comment
    # (lib/session.sh) for the full reasoning -- --app mode hides the tab
    # strip/address bar so it doesn't
    ...
    # silently downloads the file instead of displaying it -- so the actual
    # "open" action below is a generated viewer page, not letting this
    # link's own href navigate.
...
echo "This script's own header comment documents fallback point_at_browser_text"
echo "coordinates (used only if a CDP lookup fails) -- watch the video and expect"
echo "to need to fix those up or adjust pacing before treating it as final."
